...
export const setupVirtualOperationViews = async () => {
  ...
  // deno-fmt-ignore
  const VOInterest =
    `CREATE OR REPLACE VIEW hafsql.vo_interest
    AS SELECT o.id as op_id,
      hb.created_at AS "timestamp",
      ${param('owner')} AS owner,
  ...
  await client.queryObject(VOInterest)
  ...
  // +33
  const VOSystemWarning = `CREATE OR REPLACE VIEW hafsql.vo_system_warning
    AS SELECT o.id as op_id,
      hb.created_at AS "timestamp",
  ...
  await client.queryObject(VOSystemWarning)
