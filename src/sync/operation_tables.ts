...
import { getOpName } from '../helpers/operation_id.ts'
...
const getData = async (blockRange: number[]) => {
  ...
  const result = await client.queryObject<Operations>(
    `SELECT id, body_binary::jsonb AS body, hive.operation_id_to_type_id(id) AS op_type_id,
      (SELECT created_at FROM hive.blocks WHERE num = hive.operation_id_to_block_num(id))
...
const processData = async (
  ...
  using client = await pool.connect()
  const trx = client.createTransaction('hafsql_operations_sync')
  await trx.begin()
  ...
  await bulkInsertData(operations, trx)
  await updateLastBlockNum('operations', blockRange[1], trx)
  await trx.commit()
}
...
const bulkInsertData = async (
  ...
  for (const op_type_id in operations) {
    ...
    if (op_name === 'vote') {
      ...
      await fillTransfer(ops, trx)
    } else if (op_name === 'transfer_to_vesting') {
      await fillTransferToVesting(ops, trx)
    } else if (op_name === 'withdraw_vesting') {
      await fillWithdrawVesting(ops, trx)
    } else if (op_name === 'limit_order_create') {
      await fillLimitOrderCreate(ops, trx)
    } else if (op_name === 'limit_order_cancel') {
      await fillLimitOrderCancel(ops, trx)
    } else if (op_name === 'feed_publish') {
      await fillFeedPublished(ops, trx)
    } else if (op_name === 'convert') {
      await fillConvert(ops, trx)
    } else if (op_name === 'account_create') {
      await fillAccountCreate(ops, trx)
    } else if (op_name === 'account_update') {
      await fillAccountUpdate(ops, trx)
    } else if (op_name === 'witness_update') {
      await fillWitnessUpdate(ops, trx)
    } else if (op_name === 'account_witness_vote') {
      await fillAccountWitnessVote(ops, trx)
    } else if (op_name === 'account_witness_proxy') {
      await fillAccountWitnessProxy(ops, trx)
    } else if (op_name === 'pow') {
      await fillPow(ops, trx)
    } else if (op_name === 'custom') {
      await fillCustom(ops, trx)
    } else if (op_name === 'delete_comment') {
      await fillDeleteComment(ops, trx)
    } else if (op_name === 'custom_json') {
      // 672GB storage used - non-compressed
      // await fillCustomJson(ops, trx)
    } else if (op_name === 'comment_options') {
      await fillCommentOptions(ops, trx)
    } else if (op_name === 'set_withdraw_vesting_route') {
      await fillSetWithdrawVestingRoute(ops, trx)
    } else if (op_name === 'limit_order_create2') {
      await fillLimitOrderCreate2(ops, trx)
    } else if (op_name === 'claim_account') {
      await fillClaimAccount(ops, trx)
    } else if (op_name === 'create_claimed_account') {
      await fillCreateClaimedAccount(ops, trx)
    } else if (op_name === 'request_account_recovery') {
      await fillRequestAccountRecovery(ops, trx)
    } else if (op_name === 'recover_account') {
      await fillRecoverAccount(ops, trx)
    } else if (op_name === 'change_recovery_account') {
      await fillChangeRecoveryAccount(ops, trx)
    } else if (op_name === 'escrow_transfer') {
      await fillEscrowTransfer(ops, trx)
    } else if (op_name === 'escrow_dispute') {
      await fillEscrowDispute(ops, trx)
    } else if (op_name === 'escrow_release') {
      await fillEscrowRelease(ops, trx)
    } else if (op_name === 'pow2') {
      await fillPow2(ops, trx)
    } else if (op_name === 'escrow_approve') {
      await fillEscrowApprove(ops, trx)
    } else if (op_name === 'transfer_to_savings') {
      await fillTransferToSavings(ops, trx)
    } else if (op_name === 'transfer_from_savings') {
      await fillTransferFromSavings(ops, trx)
    } else if (op_name === 'cancel_transfer_from_savings') {
      await fillCancelTransferFromSavings(ops, trx)
    } else if (op_name === 'decline_voting_rights') {
      await fillDeclineVotingRights(ops, trx)
    } else if (op_name === 'claim_reward_balance') {
      await fillClaimRewardBalance(ops, trx)
    } else if (op_name === 'delegate_vesting_shares') {
      await fillDelegateVestingShares(ops, trx)
    } else if (op_name === 'account_create_with_delegation') {
      await fillAccountCreateWithDelegation(ops, trx)
    } else if (op_name === 'witness_set_properties') {
      await fillWitnessSetProperties(ops, trx)
    } else if (op_name === 'account_update2') {
      await fillAccountUpdate2(ops, trx)
    } else if (op_name === 'create_proposal') {
      await fillCreateProposal(ops, trx)
    } else if (op_name === 'update_proposal_votes') {
      await fillUpdateProposalVotes(ops, trx)
    } else if (op_name === 'remove_proposal') {
      await fillRemoveProposal(ops, trx)
    } else if (op_name === 'update_proposal') {
      await fillUpdateProposal(ops, trx)
    } else if (op_name === 'collateralized_convert') {
      await fillCollateralizedConvert(ops, trx)
    } else if (op_name === 'recurrent_transfer') {
      await fillRecurrentTransfer(ops, trx)
    } else if (op_name === 'fill_convert_request') { // VOPs
      await fillFillConvertRequest(ops, trx)
    } else if (op_name === 'author_reward') {
      await fillAuthorReward(ops, trx)
    } else if (op_name === 'curation_reward') {
      await fillCurationReward(ops, trx)
    } else if (op_name === 'comment_reward') {
      await fillCommentReward(ops, trx)
    } else if (op_name === 'liquidity_reward') {
      await fillLiquidityReward(ops, trx)
    } else if (op_name === 'interest') {
      await fillInterest(ops, trx)
    } else if (op_name === 'fill_vesting_withdraw') {
      await fillFillVestingWithdraw(ops, trx)
    } else if (op_name === 'fill_order') {
      await fillFillOrder(ops, trx)
    } else if (op_name === 'shutdown_witness') {
      await fillShutdownWitness(ops, trx)
    } else if (op_name === 'fill_transfer_from_savings') {
      await fillFillTransferFromSavings(ops, trx)
    } else if (op_name === 'hardfork') {
      await fillHardfork(ops, trx)
    } else if (op_name === 'comment_payout_update') {
      await fillCommentPayoutUpdate(ops, trx)
    } else if (op_name === 'return_vesting_delegation') {
      await fillReturnVestingDelegation(ops, trx)
    } else if (op_name === 'comment_benefactor_reward') {
      await fillCommentBenefactorReward(ops, trx)
    } else if (op_name === 'producer_reward') {
      await fillProducerReward(ops, trx)
    } else if (op_name === 'clear_null_account_balance') {
      await fillClearNullAccountBalance(ops, trx)
    } else if (op_name === 'proposal_pay') {
      await fillProposalPay(ops, trx)
    } else if (op_name === 'dhf_funding') {
      await fillDhfFunding(ops, trx)
    } else if (op_name === 'hardfork_hive') {
      await fillHardforkHive(ops, trx)
    } else if (op_name === 'hardfork_hive_restore') {
      await fillHardforkHiveRestore(ops, trx)
    } else if (op_name === 'delayed_voting') {
      await fillDelayedVoting(ops, trx)
    } else if (op_name === 'consolidate_treasury_balance') {
      await fillConsolidateTreasuryBalance(ops, trx)
    } else if (op_name === 'effective_comment_vote') {
      // 159GB storage used - non-compressed
      // await fillEffectiveCommentVote(ops, trx)
    } else if (op_name === 'ineffective_delete_comment') {
      await fillIneffectiveDeleteComment(ops, trx)
    } else if (op_name === 'dhf_conversion') {
      await fillDhfConversion(ops, trx)
    } else if (op_name === 'expired_account_notification') {
      await fillExpiredAccountNotification(ops, trx)
    } else if (op_name === 'changed_recovery_account') {
      await fillChangedRecoveryAccount(ops, trx)
    } else if (op_name === 'transfer_to_vesting_completed') {
      await fillTransferToVestingCompleted(ops, trx)
    } else if (op_name === 'pow_reward') {
      await fillPowReward(ops, trx)
    } else if (op_name === 'vesting_shares_split') {
      await fillVestingSharesSplit(ops, trx)
    } else if (op_name === 'account_created') {
      await fillAccountCreated(ops, trx)
    } else if (op_name === 'fill_collateralized_convert_request') {
      await fillFillCollateralizedConvertRequest(ops, trx)
    } else if (op_name === 'system_warning') {
      await fillSystemWarning(ops, trx)
    } else if (op_name === 'fill_recurrent_transfer') {
      await fillFillRcurrentTransfer(ops, trx)
    } else if (op_name === 'failed_recurrent_transfer') {
      await fillFailedRecurrentTransfer(ops, trx)
    } else if (op_name === 'limit_order_cancelled') {
      await fillLimitOrderCancelled(ops, trx)
    } else if (op_name === 'producer_missed') {
      await fillProducerMissed(ops, trx)
    } else if (op_name === 'proposal_fee') {
      await fillProposalFee(ops, trx)
    } else if (op_name === 'collateralized_convert_immediate_conversion') {
      await fillCollateralizedConvertImmediateConversion(ops, trx)
    } else if (op_name === 'escrow_approved') {
      await fillEscrowApproved(ops, trx)
    } else if (op_name === 'escrow_rejected') {
      await fillEscrowRejected(ops, trx)
    } else if (op_name === 'proxy_cleared') {
      await fillProxyCleared(ops, trx)
    }
  }
}
...
const fillVote = async (ops: Operations[], trx: Transaction) => {
  ...
  for (let i = 0; i < ops.length; i++) {
    ...
    ids.push(ops[i].id)
    timestamps.push(ops[i].created_at)
  }
  await trx
    .queryObject`INSERT INTO hafsql.operation_vote_table (id, timestamp, voter, author, permlink, weight)
      SELECT UNNEST(${ids}::int8[]), UNNEST(${timestamps}::timestamp[]), UNNEST(${voters}::text[]),
...
const fillComment = async (ops: Operations[], trx: Transaction) => {
  ...
  await trx.queryObject`INSERT INTO hafsql.operation_comment_table
    (id, timestamp, author, permlink, parent_author, parent_permlink, title, body, json_metadata)
    SELECT UNNEST(${ids}::int8[]), UNNEST(${timestamps}::timestamp[]), UNNEST(${authors}::text[]),
...
const fillTransfer = async (ops: Operations[], trx: Transaction) => {
  ...
  await trx.queryObject`INSERT INTO hafsql.operation_transfer_table
    (id, timestamp, from_account, to_account, amount, symbol, memo)
    SELECT UNNEST(${ids}::int8[]), UNNEST(${timestamps}::timestamp[]), UNNEST(${froms}::text[]), UNNEST(${tos}::text[]),
    hafsql.asset_amount(UNNEST(${amounts}::text[])), hafsql.asset_symbol(UNNEST(${amounts}::text[])), UNNEST(${memos}::text[])
    ON CONFLICT (id) DO NOTHING;`
}

const fillTransferToVesting = async (ops: Operations[], trx: Transaction) => {
  const ids = []
  const timestamps = []
  const froms = []
  const tos = []
  ...
  await trx.queryObject`INSERT INTO hafsql.operation_transfer_to_vesting_table
    (id, timestamp, from_account, to_account, amount, symbol)
    SELECT UNNEST(${ids}::int8[]), UNNEST(${timestamps}::timestamp[]), UNNEST(${froms}::text[]), UNNEST(${tos}::text[]),
    hafsql.asset_amount(UNNEST(${amounts}::text[])), hafsql.asset_symbol(UNNEST(${amounts}::text[]))
    ON CONFLICT (id) DO NOTHING;`
}

const fillWithdrawVesting = async (ops: Operations[], trx: Transaction) => {
  const accounts = []
  const vests = []
  const ids = []
  const timestamps = []
  ...
  await trx.queryObject`INSERT INTO hafsql.operation_withdraw_vesting_table
    (id, timestamp, account, vesting_shares, symbol)
    SELECT UNNEST(${ids}::int8[]), UNNEST(${timestamps}::timestamp[]), UNNEST(${accounts}::text[]),
    hafsql.asset_amount(UNNEST(${vests}::text[])), hafsql.asset_symbol(UNNEST(${vests}::text[]))
    ON CONFLICT (id) DO NOTHING;`
}

const fillLimitOrderCreate = async (ops: Operations[], trx: Transaction) => {
  const ids = []
  const timestamps = []
  const owners = []
  const orderids = []
  ...
  await trx.queryObject`INSERT INTO hafsql.operation_limit_order_create_table
    (id, timestamp, owner, orderid, expiration, amount_to_sell, amount_to_sell_symbol,
      min_to_receive, min_to_receive_symbol, fill_or_kill)
    SELECT UNNEST(${ids}::int8[]), UNNEST(${timestamps}::timestamp[]),
    UNNEST(${owners}::text[]),
...
const fillLimitOrderCancel = async (ops: Operations[], trx: Transaction) => {
  const ids = []
  const timestamps = []
  const owners = []
  const orderids = []
  ...
  await trx.queryObject`INSERT INTO hafsql.operation_limit_order_cancel_table
    (id, timestamp, owner, orderid)
    SELECT UNNEST(${ids}::int8[]), UNNEST(${timestamps}::timestamp[]),
    UNNEST(${owners}::text[]), UNNEST(${orderids}::int8[])
    ON CONFLICT (id) DO NOTHING;`
}

const fillFeedPublished = async (ops: Operations[], trx: Transaction) => {
  const ids = []
  const timestamps = []
  const publishers = []
  const rates = []
  ...
  for (let i = 0; i < ops.length; i++) {
    ...
    rates.push(value.exchange_rate)
    quotes.push(value.quote)
  }
  await trx.queryObject`INSERT INTO hafsql.operation_feed_publish_table
    (id, timestamp, publisher, exchange_rate_base_amount, exchange_rate_base_symbol,
      exchange_rate_quote_amount, exchange_rate_quote_symbol)
    SELECT UNNEST(${ids}::int8[]), UNNEST(${timestamps}::timestamp[]),
    UNNEST(${publishers}::text[]),
    hafsql.asset_amount(UNNEST(${rates}::text[])),
    hafsql.asset_symbol(UNNEST(${rates}::text[])),
    hafsql.asset_amount(UNNEST(${quotes}::text[])),
    hafsql.asset_symbol(UNNEST(${quotes}::text[]))
    ON CONFLICT (id) DO NOTHING;`
}

const fillConvert = async (ops: Operations[], trx: Transaction) => {
  const ids = []
  const timestamps = []
  const owners = []
  const requestids = []
  ...
  await trx.queryObject`INSERT INTO hafsql.operation_convert_table
    (id, timestamp, owner, requestid, amount, symbol)
    SELECT UNNEST(${ids}::int8[]), UNNEST(${timestamps}::timestamp[]),
...
const fillAccountCreate = async (ops: Operations[], trx: Transaction) => {
  const ids = []
  const timestamps = []
  const fees = []
  const creators = []
  ...
  await trx.queryObject`INSERT INTO hafsql.operation_account_create_table
    (id, timestamp, fee, fee_symbol, creator, new_account_name, owner,
...
const fillAccountUpdate = async (ops: Operations[], trx: Transaction) => {
  const ids = []
  const timestamps = []
  const accounts = []
  const owners = []
  ...
  await trx.queryObject`INSERT INTO hafsql.operation_account_update_table
    (id, timestamp, account, owner, active, posting, memo_key, json_metadata)
    SELECT UNNEST(${ids}::int8[]), UNNEST(${timestamps}::timestamp[]),
...
const fillWitnessUpdate = async (ops: Operations[], trx: Transaction) => {
  const ids = []
  const timestamps = []
  const owners = []
  const urls = []
  ...
  await trx.queryObject`INSERT INTO hafsql.operation_witness_update_table
    (id, timestamp, owner, url, block_signing_key, props_hbd_interest_rate,
...
const fillAccountWitnessVote = async (ops: Operations[], trx: Transaction) => {
  const ids = []
  const timestamps = []
  const accounts = []
  const witnesses = []
  ...
  await trx.queryObject`INSERT INTO hafsql.operation_account_witness_vote_table
    (id, timestamp, account, witness, approve)
    SELECT UNNEST(${ids}::int8[]), UNNEST(${timestamps}::timestamp[]),
...
const fillAccountWitnessProxy = async (
  ...
  await trx.queryObject`INSERT INTO hafsql.operation_account_witness_proxy_table
    (id, timestamp, account, proxy)
    SELECT UNNEST(${ids}::int8[]), UNNEST(${timestamps}::timestamp[]),
    UNNEST(${accounts}::text[]), UNNEST(${proxys}::text[])
    ON CONFLICT (id) DO NOTHING;`
}

const fillPow = async (ops: Operations[], trx: Transaction) => {
  const ids = []
  const timestamps = []
  const accounts = []
  const blockids = []
  ...
  await trx.queryObject`INSERT INTO hafsql.operation_pow_table
    (id, timestamp, worker_account, block_id, nonce, work, props)
    SELECT UNNEST(${ids}::int8[]), UNNEST(${timestamps}::timestamp[]),
...
const fillCustom = async (ops: Operations[], trx: Transaction) => {
  const ids = []
  const timestamps = []
  const auths = []
  const cids = []
  ...
  await trx.queryObject`INSERT INTO hafsql.operation_custom_table
    (id, timestamp, required_auths, custom_id, data)
    SELECT UNNEST(${ids}::int8[]), UNNEST(${timestamps}::timestamp[]),
    UNNEST(${auths}::jsonb[]), UNNEST(${cids}::int8[]), UNNEST(${datas}::text[])
    ON CONFLICT (id) DO NOTHING;`
}

const fillDeleteComment = async (ops: Operations[], trx: Transaction) => {
  const ids = []
  const timestamps = []
  const authors = []
  const permlinks = []
  ...
  await trx.queryObject`INSERT INTO hafsql.operation_delete_comment_table
    (id, timestamp, author, permlink)
    SELECT UNNEST(${ids}::int8[]), UNNEST(${timestamps}::timestamp[]),
...
const fillCustomJson = async (ops: Operations[], trx: Transaction) => {
  ...
  await trx.queryObject`INSERT INTO hafsql.operation_custom_json_table
    (id, timestamp, required_auths, required_posting_auths, custom_id, json)
    SELECT UNNEST(${ids}::int8[]), UNNEST(${timestamps}::timestamp[]),
...
const fillCommentOptions = async (ops: Operations[], trx: Transaction) => {
  ...
  await trx.queryObject`INSERT INTO hafsql.operation_comment_options_table
    (id, timestamp, author, permlink, max_accepted_payout, max_accepted_payout_symbol,
...
const fillSetWithdrawVestingRoute = async (
  ...
  await trx
    .queryObject`INSERT INTO hafsql.operation_set_withdraw_vesting_route_table
    (id, timestamp, from_account, to_account, percent, auto_vest)
    SELECT UNNEST(${ids}::int8[]), UNNEST(${timestamps}::timestamp[]),
...
const fillLimitOrderCreate2 = async (ops: Operations[], trx: Transaction) => {
  const ids = []
  const timestamps = []
  const owners = []
  const orderids = []
  ...
  await trx.queryObject`INSERT INTO hafsql.operation_limit_order_create2_table
    (id, timestamp, owner, orderid, amount_to_sell, amount_to_sell_symbol,
...
const fillClaimAccount = async (ops: Operations[], trx: Transaction) => {
  const ids = []
  const timestamps = []
  const creators = []
  const fees = []
  ...
  await trx.queryObject`INSERT INTO hafsql.operation_claim_account_table
    (id, timestamp, creator, fee, symbol, extensions)
    SELECT UNNEST(${ids}::int8[]), UNNEST(${timestamps}::timestamp[]),
...
const fillCreateClaimedAccount = async (
  ...
  await trx
    .queryObject`INSERT INTO hafsql.operation_create_claimed_account_table
    (id, timestamp, creator, new_account_name, owner, active, posting,
...
const fillRequestAccountRecovery = async (
  ...
  await trx
    .queryObject`INSERT INTO hafsql.operation_request_account_recovery_table
    (id, timestamp, recovery_account, account_to_recover, new_owner_authority, extensions)
    SELECT UNNEST(${ids}::int8[]), UNNEST(${timestamps}::timestamp[]),
...
const fillRecoverAccount = async (ops: Operations[], trx: Transaction) => {
  const ids = []
  const timestamps = []
  const accounts = []
  const newOwners = []
  ...
  await trx.queryObject`INSERT INTO hafsql.operation_recover_account_table
    (id, timestamp, account_to_recover, new_owner_authority, recent_owner_authority, extensions)
    SELECT UNNEST(${ids}::int8[]), UNNEST(${timestamps}::timestamp[]),
...
const fillChangeRecoveryAccount = async (
  ...
  await trx
    .queryObject`INSERT INTO hafsql.operation_change_recovery_account_table
    (id, timestamp, account_to_recover, new_recovery_account, extensions)
    SELECT UNNEST(${ids}::int8[]), UNNEST(${timestamps}::timestamp[]),
...
const fillEscrowTransfer = async (ops: Operations[], trx: Transaction) => {
  const ids = []
  const timestamps = []
  const froms = []
  const tos = []
  ...
  await trx.queryObject`INSERT INTO hafsql.operation_escrow_transfer_table
    (id, timestamp, from_account, to_account, hbd_amount, hive_amount, escrow_id, agent,
...
const fillEscrowDispute = async (ops: Operations[], trx: Transaction) => {
  const ids = []
  const timestamps = []
  const froms = []
  const tos = []
  ...
  await trx.queryObject`INSERT INTO hafsql.operation_escrow_dispute_table
    (id, timestamp, from_account, to_account, agent, who, escrow_id)
    SELECT UNNEST(${ids}::int8[]), UNNEST(${timestamps}::timestamp[]),
    UNNEST(${froms}::text[]), UNNEST(${tos}::text[]),
...
const fillEscrowRelease = async (ops: Operations[], trx: Transaction) => {
  const ids = []
  const timestamps = []
  const froms = []
  const tos = []
  ...
  await trx.queryObject`INSERT INTO hafsql.operation_escrow_release_table
    (id, timestamp, from_account, to_account, agent, who, receiver,
...
const fillPow2 = async (ops: Operations[], trx: Transaction) => {
  const ids = []
  const timestamps = []
  const works = []
  const props = []
  ...
  await trx.queryObject`INSERT INTO hafsql.operation_pow2_table
    (id, timestamp, work, props)
    SELECT UNNEST(${ids}::int8[]), UNNEST(${timestamps}::timestamp[]),
    UNNEST(${works}::jsonb[]), UNNEST(${props}::jsonb[])
    ON CONFLICT (id) DO NOTHING;`
}

const fillEscrowApprove = async (ops: Operations[], trx: Transaction) => {
  const ids = []
  const timestamps = []
  const froms = []
  const tos = []
  ...
  await trx.queryObject`INSERT INTO hafsql.operation_escrow_approve_table
    (id, timestamp, from_account, to_account, agent, who, escrow_id, approve)
    SELECT UNNEST(${ids}::int8[]), UNNEST(${timestamps}::timestamp[]),
    UNNEST(${froms}::text[]), UNNEST(${tos}::text[]),
    UNNEST(${agents}::text[]), UNNEST(${whos}::text[]),
...
const fillTransferToSavings = async (ops: Operations[], trx: Transaction) => {
  const ids = []
  const timestamps = []
  const froms = []
  const tos = []
  ...
  await trx.queryObject`INSERT INTO hafsql.operation_transfer_to_savings_table
    (id, timestamp, from_account, to_account, amount, symbol, memo)
    SELECT UNNEST(${ids}::int8[]), UNNEST(${timestamps}::timestamp[]),
    UNNEST(${froms}::text[]), UNNEST(${tos}::text[]),
    hafsql.asset_amount(UNNEST(${amounts}::text[])),
...
const fillTransferFromSavings = async (
  ...
  await trx.queryObject`INSERT INTO hafsql.operation_transfer_from_savings_table
    (id, timestamp, from_account, to_account, request_id, amount, symbol, memo)
    SELECT UNNEST(${ids}::int8[]), UNNEST(${timestamps}::timestamp[]),
...
const fillCancelTransferFromSavings = async (
  ...
  await trx
    .queryObject`INSERT INTO hafsql.operation_cancel_transfer_from_savings_table
    (id, timestamp, from_account, request_id)
    SELECT UNNEST(${ids}::int8[]), UNNEST(${timestamps}::timestamp[]),
...
const fillDeclineVotingRights = async (
  ...
  await trx.queryObject`INSERT INTO hafsql.operation_decline_voting_rights_table
    (id, timestamp, account, decline)
    SELECT UNNEST(${ids}::int8[]), UNNEST(${timestamps}::timestamp[]),
    UNNEST(${accounts}::text[]), UNNEST(${declines}::boolean[])
    ON CONFLICT (id) DO NOTHING;`
}

const fillClaimRewardBalance = async (ops: Operations[], trx: Transaction) => {
  const ids = []
  const timestamps = []
  const accounts = []
  const hives = []
  ...
  await trx.queryObject`INSERT INTO hafsql.operation_claim_reward_balance_table
    (id, timestamp, account, reward_hive, reward_hbd, reward_vests)
    SELECT UNNEST(${ids}::int8[]), UNNEST(${timestamps}::timestamp[]),
...
const fillDelegateVestingShares = async (
  ...
  await trx
    .queryObject`INSERT INTO hafsql.operation_delegate_vesting_shares_table
    (id, timestamp, delegator, delegatee, vesting_shares)
    SELECT UNNEST(${ids}::int8[]), UNNEST(${timestamps}::timestamp[]),
...
const fillAccountCreateWithDelegation = async (
  ...
  await trx
    .queryObject`INSERT INTO hafsql.operation_account_create_with_delegation_table
    (id, timestamp, creator, new_account_name, fee, fee_symbol, delegation, owner,
...
const fillWitnessSetProperties = async (
  ...
  await trx
    .queryObject`INSERT INTO hafsql.operation_witness_set_properties_table
    (id, timestamp, owner, props, extensions)
    SELECT UNNEST(${ids}::int8[]), UNNEST(${timestamps}::timestamp[]),
    UNNEST(${owners}::text[]), UNNEST(${props}::jsonb[]), UNNEST(${exes}::jsonb[])
    ON CONFLICT (id) DO NOTHING;`
}

const fillAccountUpdate2 = async (ops: Operations[], trx: Transaction) => {
  const ids = []
  const timestamps = []
  const accounts = []
  const metadatas = []
  ...
  await trx.queryObject`INSERT INTO hafsql.operation_account_update2_table
    (id, timestamp, account, json_metadata, posting_json_metadata, extensions)
    SELECT UNNEST(${ids}::int8[]), UNNEST(${timestamps}::timestamp[]),
    UNNEST(${accounts}::text[]), hafsql.to_json(UNNEST(${metadatas}::text[])),
    hafsql.to_json(UNNEST(${accounts}::text[])), UNNEST(${exes}::jsonb[])
    ON CONFLICT (id) DO NOTHING;`
}

const fillCreateProposal = async (ops: Operations[], trx: Transaction) => {
  const ids = []
  const timestamps = []
  const creators = []
  const receivers = []
  ...
  await trx.queryObject`INSERT INTO hafsql.operation_create_proposal_table
    (id, timestamp, creator, receiver, subject, permlink, start_date, end_date,
...
const fillUpdateProposalVotes = async (
  ...
  await trx.queryObject`INSERT INTO hafsql.operation_update_proposal_votes_table
    (id, timestamp, voter, proposal_ids, approve, extensions)
    SELECT UNNEST(${ids}::int8[]), UNNEST(${timestamps}::timestamp[]),
...
const fillRemoveProposal = async (ops: Operations[], trx: Transaction) => {
  const ids = []
  const timestamps = []
  const owners = []
  const pids = []
  ...
  await trx.queryObject`INSERT INTO hafsql.operation_remove_proposal_table
    (id, timestamp, proposal_owner, proposal_ids, extensions)
    SELECT UNNEST(${ids}::int8[]), UNNEST(${timestamps}::timestamp[]),
    UNNEST(${owners}::text[]), UNNEST(${pids}::jsonb[]), UNNEST(${exes}::jsonb[])
    ON CONFLICT (id) DO NOTHING;`
}

const fillUpdateProposal = async (ops: Operations[], trx: Transaction) => {
  const ids = []
  const timestamps = []
  const pids = []
  const creators = []
  ...
  await trx.queryObject`INSERT INTO hafsql.operation_update_proposal_table
    (id, timestamp, proposal_id, creator, daily_pay, daily_pay_symbol,
...
const fillCollateralizedConvert = async (
  ...
  await trx
    .queryObject`INSERT INTO hafsql.operation_collateralized_convert_table
    (id, timestamp, owner, requestid, amount, symbol)
    SELECT UNNEST(${ids}::int8[]), UNNEST(${timestamps}::timestamp[]),
...
const fillRecurrentTransfer = async (ops: Operations[], trx: Transaction) => {
  const ids = []
  const timestamps = []
  const froms = []
  const tos = []
  ...
  await trx.queryObject`INSERT INTO hafsql.operation_recurrent_transfer_table
    (id, timestamp, from_account, to_account, amount, symbol, memo,
...
const fillFillConvertRequest = async (ops: Operations[], trx: Transaction) => {
  const ids = []
  const timestamps = []
  const owners = []
  const rids = []
  ...
  await trx.queryObject`INSERT INTO hafsql.operation_fill_convert_request_table
    (id, timestamp, owner, requestid, amount_in, amount_in_symbol,
...
const fillAuthorReward = async (ops: Operations[], trx: Transaction) => {
  const ids = []
  const timestamps = []
  const authors = []
  const permlinks = []
  ...
  await trx.queryObject`INSERT INTO hafsql.operation_author_reward_table
    (id, timestamp, author, permlink, hbd_payout, hive_payout, vesting_payout,
...
const fillCurationReward = async (ops: Operations[], trx: Transaction) => {
  const ids = []
  const timestamps = []
  const curators = []
  const rewards = []
  ...
  await trx.queryObject`INSERT INTO hafsql.operation_curation_reward_table
    (id, timestamp, curator, reward, symbol, author, permlink, payout_must_be_claimed)
    SELECT UNNEST(${ids}::int8[]), UNNEST(${timestamps}::timestamp[]),
...
const fillCommentReward = async (ops: Operations[], trx: Transaction) => {
  const ids = []
  const timestamps = []
  const authors = []
  const permlinks = []
  ...
  await trx.queryObject`INSERT INTO hafsql.operation_comment_reward_table
    (id, timestamp, author, permlink, payout, author_rewards, total_payout_value,
      curator_payout_value, beneficiary_payout_value)
    SELECT UNNEST(${ids}::int8[]), UNNEST(${timestamps}::timestamp[]),
    UNNEST(${authors}::text[]), UNNEST(${permlinks}::text[]),
    hafsql.asset_amount(UNNEST(${payouts}::text[])),
    UNNEST(${authorRewards}::numeric[]),
...
const fillLiquidityReward = async (ops: Operations[], trx: Transaction) => {
  const ids = []
  const timestamps = []
  const owners = []
  const payouts = []
  ...
  await trx.queryObject`INSERT INTO hafsql.operation_liquidity_reward_table
    (id, timestamp, owner, payout, symbol)
    SELECT UNNEST(${ids}::int8[]), UNNEST(${timestamps}::timestamp[]),
    UNNEST(${owners}::text[]),
    hafsql.asset_amount(UNNEST(${payouts}::text[])),
...
const fillInterest = async (ops: Operations[], trx: Transaction) => {
  const ids = []
  const timestamps = []
  const owners = []
  const interests = []
  ...
  await trx.queryObject`INSERT INTO hafsql.operation_interest_table
    (id, timestamp, owner, interest, interest_symbol, is_saved_into_hbd_balance)
    SELECT UNNEST(${ids}::int8[]), UNNEST(${timestamps}::timestamp[]),
    UNNEST(${owners}::text[]),
...
const fillFillVestingWithdraw = async (
  ...
  await trx.queryObject`INSERT INTO hafsql.operation_fill_vesting_withdraw_table
    (id, timestamp, from_account, to_account, withdrawn, withdrawn_symbol,
...
const fillFillOrder = async (ops: Operations[], trx: Transaction) => {
  const ids = []
  const timestamps = []
  const cowners = []
  const oowners = []
  ...
  await trx.queryObject`INSERT INTO hafsql.operation_fill_order_table
    (id, timestamp, current_owner, open_owner, current_orderid, open_orderid,
...
const fillShutdownWitness = async (ops: Operations[], trx: Transaction) => {
  const ids = []
  const timestamps = []
  const owners = []
  for (let i = 0; i < ops.length; i++) {
  ...
  await trx.queryObject`INSERT INTO hafsql.operation_shutdown_witness_table
    (id, timestamp, owner)
    SELECT UNNEST(${ids}::int8[]), UNNEST(${timestamps}::timestamp[]),
    UNNEST(${owners}::text[])
    ON CONFLICT (id) DO NOTHING;`
...
const fillFillTransferFromSavings = async (
  ...
  await trx
    .queryObject`INSERT INTO hafsql.operation_fill_transfer_from_savings_table
    (id, timestamp, from_account, to_account, request_id, memo, amount, symbol)
    SELECT UNNEST(${ids}::int8[]), UNNEST(${timestamps}::timestamp[]),
    UNNEST(${froms}::text[]), UNNEST(${tos}::text[]),
...
const fillHardfork = async (ops: Operations[], trx: Transaction) => {
  const ids = []
  const timestamps = []
  const hids = []
  for (let i = 0; i < ops.length; i++) {
  ...
  await trx.queryObject`INSERT INTO hafsql.operation_hardfork_table
    (id, timestamp, hardfork_id)
    SELECT UNNEST(${ids}::int8[]), UNNEST(${timestamps}::timestamp[]),
    UNNEST(${hids}::int2[])
    ON CONFLICT (id) DO NOTHING;`
}
...
const fillCommentPayoutUpdate = async (
  ...
  await trx.queryObject`INSERT INTO hafsql.operation_comment_payout_update_table
    (id, timestamp, author, permlink)
    SELECT UNNEST(${ids}::int8[]), UNNEST(${timestamps}::timestamp[]),
    UNNEST(${authors}::text[]), UNNEST(${permlinks}::text[])
    ON CONFLICT (id) DO NOTHING;`
...
const fillReturnVestingDelegation = async (
  ...
  await trx
    .queryObject`INSERT INTO hafsql.operation_return_vesting_delegation_table
    (id, timestamp, account, vesting_shares)
    SELECT UNNEST(${ids}::int8[]), UNNEST(${timestamps}::timestamp[]),
    UNNEST(${accounts}::text[]),
    hafsql.asset_amount(UNNEST(${vests}::text[]))
    ON CONFLICT (id) DO NOTHING;`
...
const fillCommentBenefactorReward = async (
  ...
  await trx
    .queryObject`INSERT INTO hafsql.operation_comment_benefactor_reward_table
    (id, timestamp, benefactor, author, permlink, hbd_payout, hive_payout,
...
const fillProducerReward = async (ops: Operations[], trx: Transaction) => {
  const ids = []
  const timestamps = []
  const producers = []
  const vests = []
  ...
  await trx.queryObject`INSERT INTO hafsql.operation_producer_reward_table
    (id, timestamp, producer, vesting_shares, vesting_shares_symbol)
    SELECT UNNEST(${ids}::int8[]), UNNEST(${timestamps}::timestamp[]),
...
const fillClearNullAccountBalance = async (
  ...
  await trx
    .queryObject`INSERT INTO hafsql.operation_clear_null_account_balance_table
    (id, timestamp, total_cleared)
    SELECT UNNEST(${ids}::int8[]), UNNEST(${timestamps}::timestamp[]),
    UNNEST(${cleared}::jsonb[])
    ON CONFLICT (id) DO NOTHING;`
}

const fillProposalPay = async (ops: Operations[], trx: Transaction) => {
  const ids = []
  const timestamps = []
  const pids = []
  const receivers = []
  ...
  await trx.queryObject`INSERT INTO hafsql.operation_proposal_pay_table
    (id, timestamp, proposal_id, receiver, payer, payment, symbol)
    SELECT UNNEST(${ids}::int8[]), UNNEST(${timestamps}::timestamp[]),
...
const fillDhfFunding = async (ops: Operations[], trx: Transaction) => {
  const ids = []
  const timestamps = []
  const treasurys = []
  const funds = []
  ...
  await trx.queryObject`INSERT INTO hafsql.operation_dhf_funding_table
    (id, timestamp, treasury, additional_funds, symbol)
    SELECT UNNEST(${ids}::int8[]), UNNEST(${timestamps}::timestamp[]),
...
const fillHardforkHive = async (ops: Operations[], trx: Transaction) => {
  const ids = []
  const timestamps = []
  const accounts = []
  const treasurys = []
  ...
  await trx.queryObject`INSERT INTO hafsql.operation_hardfork_hive_table
    (id, timestamp, account, treasury, other_affected_accounts, hive_transferred,
...
const fillHardforkHiveRestore = async (
  ...
  await trx.queryObject`INSERT INTO hafsql.operation_hardfork_hive_restore_table
    (id, timestamp, account, treasury, hbd_transferred, hive_transferred)
    SELECT UNNEST(${ids}::int8[]), UNNEST(${timestamps}::timestamp[]),
    UNNEST(${accounts}::text[]), UNNEST(${treasurys}::text[]),
    hafsql.asset_amount(UNNEST(${hbds}::text[])),
    hafsql.asset_amount(UNNEST(${hives}::text[]))
...
const fillDelayedVoting = async (ops: Operations[], trx: Transaction) => {
  const ids = []
  const timestamps = []
  const voters = []
  const votes = []
  ...
  await trx.queryObject`INSERT INTO hafsql.operation_delayed_voting_table
    (id, timestamp, voter, votes)
    SELECT UNNEST(${ids}::int8[]), UNNEST(${timestamps}::timestamp[]),
...
const fillConsolidateTreasuryBalance = async (
  ...
  await trx
    .queryObject`INSERT INTO hafsql.operation_consolidate_treasury_balance_table
    (id, timestamp, total_moved)
    SELECT UNNEST(${ids}::int8[]), UNNEST(${timestamps}::timestamp[]),
    UNNEST(${totals}::jsonb[])
    ON CONFLICT (id) DO NOTHING;`
}
...
const fillEffectiveCommentVote = async (
  ...
  await trx
    ...
    (id, timestamp, voter, author, permlink, weight, rshares, total_vote_weight,
...
const fillIneffectiveDeleteComment = async (
  ...
  await trx
    .queryObject`INSERT INTO hafsql.operation_ineffective_delete_comment_table
    (id, timestamp, author, permlink)
    SELECT UNNEST(${ids}::int8[]), UNNEST(${timestamps}::timestamp[]),
    UNNEST(${authors}::text[]), UNNEST(${permlinks}::text[])
    ON CONFLICT (id) DO NOTHING;`
...
const fillDhfConversion = async (ops: Operations[], trx: Transaction) => {
  const ids = []
  const timestamps = []
  const treasurys = []
  const hives = []
  ...
  await trx.queryObject`INSERT INTO hafsql.operation_dhf_conversion_table
    (id, timestamp, treasury, hive_amount_in, hbd_amount_out)
    SELECT UNNEST(${ids}::int8[]), UNNEST(${timestamps}::timestamp[]),
    UNNEST(${treasurys}::text[]),
    hafsql.asset_amount(UNNEST(${hives}::text[])),
    hafsql.asset_amount(UNNEST(${hbds}::text[]))
...
const fillExpiredAccountNotification = async (
  ...
  await trx
    .queryObject`INSERT INTO hafsql.operation_expired_account_notification_table
...
const fillChangedRecoveryAccount = async (
  ...
  await trx
    .queryObject`INSERT INTO hafsql.operation_changed_recovery_account_table
    (id, timestamp, account, old_recovery_account, new_recovery_account)
    SELECT UNNEST(${ids}::int8[]), UNNEST(${timestamps}::timestamp[]),
...
const fillTransferToVestingCompleted = async (
  ...
  await trx
    .queryObject`INSERT INTO hafsql.operation_transfer_to_vesting_completed_table
    (id, timestamp, from_account, to_account, hive_vested, vesting_shares_received)
    SELECT UNNEST(${ids}::int8[]), UNNEST(${timestamps}::timestamp[]),
    UNNEST(${froms}::text[]), UNNEST(${tos}::text[]),
    hafsql.asset_amount(UNNEST(${hives}::text[])),
    hafsql.asset_amount(UNNEST(${vests}::text[]))
...
const fillPowReward = async (ops: Operations[], trx: Transaction) => {
  const ids = []
  const timestamps = []
  const workers = []
  const rewards = []
  ...
  await trx.queryObject`INSERT INTO hafsql.operation_pow_reward_table
    (id, timestamp, worker, reward, symbol)
    SELECT UNNEST(${ids}::int8[]), UNNEST(${timestamps}::timestamp[]),
    UNNEST(${workers}::text[]),
    hafsql.asset_amount(UNNEST(${rewards}::text[])),
    hafsql.asset_symbol(UNNEST(${rewards}::text[]))
...
const fillVestingSharesSplit = async (ops: Operations[], trx: Transaction) => {
  const ids = []
  const timestamps = []
  const owners = []
  const afters = []
  ...
  await trx.queryObject`INSERT INTO hafsql.operation_vesting_shares_split_table
    (id, timestamp, owner, vesting_shares_before_split, vesting_shares_after_split)
    SELECT UNNEST(${ids}::int8[]), UNNEST(${timestamps}::timestamp[]),
    UNNEST(${owners}::text[]),
...
const fillAccountCreated = async (ops: Operations[], trx: Transaction) => {
  const ids = []
  const timestamps = []
  const names = []
  const creators = []
  ...
  await trx.queryObject`INSERT INTO hafsql.operation_account_created_table
    (id, timestamp, new_account_name, creator, initial_vesting_shares, initial_delegation)
    SELECT UNNEST(${ids}::int8[]), UNNEST(${timestamps}::timestamp[]),
...
const fillFillCollateralizedConvertRequest = async (
  ...
  await trx
    .queryObject`INSERT INTO hafsql.operation_fill_collateralized_convert_request_table
    (id, timestamp, owner, requestid, amount_in, amount_in_symbol, amount_out, amount_out_symbol,
...
const fillSystemWarning = async (ops: Operations[], trx: Transaction) => {
  const ids = []
  const timestamps = []
  const messages = []
  for (let i = 0; i < ops.length; i++) {
  ...
  await trx.queryObject`INSERT INTO hafsql.operation_system_warning_table
...
const fillFillRcurrentTransfer = async (
  ...
  await trx
    .queryObject`INSERT INTO hafsql.operation_fill_recurrent_transfer_table
    (id, timestamp, from_account, to_account, amount, symbol, memo, remaining_executions)
    SELECT UNNEST(${ids}::int8[]), UNNEST(${timestamps}::timestamp[]),
    UNNEST(${froms}::text[]), UNNEST(${tos}::text[]),
    hafsql.asset_amount(UNNEST(${amounts}::text[])),
    hafsql.asset_symbol(UNNEST(${amounts}::text[])),
...
const fillFailedRecurrentTransfer = async (
  ...
  await trx
    .queryObject`INSERT INTO hafsql.operation_failed_recurrent_transfer_table
    (id, timestamp, from_account, to_account, amount, symbol, memo, consecutive_failures,
...
const fillLimitOrderCancelled = async (
  ...
  await trx.queryObject`INSERT INTO hafsql.operation_limit_order_cancelled_table
    (id, timestamp, seller, orderid, amount_back, symbol)
    SELECT UNNEST(${ids}::int8[]), UNNEST(${timestamps}::timestamp[]),
...
const fillProducerMissed = async (ops: Operations[], trx: Transaction) => {
  const ids = []
  const timestamps = []
  const producers = []
  for (let i = 0; i < ops.length; i++) {
  ...
  await trx.queryObject`INSERT INTO hafsql.operation_producer_missed_table
    (id, timestamp, producer)
    SELECT UNNEST(${ids}::int8[]), UNNEST(${timestamps}::timestamp[]),
    UNNEST(${producers}::text[])
    ON CONFLICT (id) DO NOTHING;`
}

const fillProposalFee = async (ops: Operations[], trx: Transaction) => {
  const ids = []
  const timestamps = []
  const creators = []
  const treasurys = []
  ...
  await trx.queryObject`INSERT INTO hafsql.operation_proposal_fee_table
    (id, timestamp, creator, treasury, proposal_id, fee, fee_symbol)
    SELECT UNNEST(${ids}::int8[]), UNNEST(${timestamps}::timestamp[]),
...
const fillCollateralizedConvertImmediateConversion = async (
  ...
  await trx
    .queryObject`INSERT INTO hafsql.operation_collateralized_convert_immediate_conversion_table
    (id, timestamp, owner, requestid, hbd_out)
    SELECT UNNEST(${ids}::int8[]), UNNEST(${timestamps}::timestamp[]),
    UNNEST(${owners}::text[]), UNNEST(${rids}::int8[]),
    hafsql.asset_amount(UNNEST(${hbds}::text[]))
    ON CONFLICT (id) DO NOTHING;`
...
const fillEscrowApproved = async (ops: Operations[], trx: Transaction) => {
  const ids = []
  const timestamps = []
  const froms = []
  const tos = []
  ...
  await trx.queryObject`INSERT INTO hafsql.operation_escrow_approved_table
    (id, timestamp, from_account, to_account, agent, escrow_id, fee, fee_symbol)
    SELECT UNNEST(${ids}::int8[]), UNNEST(${timestamps}::timestamp[]),
    UNNEST(${froms}::text[]), UNNEST(${tos}::text[]),
...
const fillEscrowRejected = async (ops: Operations[], trx: Transaction) => {
  const ids = []
  const timestamps = []
  const froms = []
  const tos = []
  ...
  await trx.queryObject`INSERT INTO hafsql.operation_escrow_rejected_table
    (id, timestamp, from_account, to_account, agent, escrow_id, hbd_amount, hive_amount,
      fee, fee_symbol)
    SELECT UNNEST(${ids}::int8[]), UNNEST(${timestamps}::timestamp[]),
    UNNEST(${froms}::text[]), UNNEST(${tos}::text[]),
    UNNEST(${agents}::text[]), UNNEST(${eids}::int8[]),
...
const fillProxyCleared = async (ops: Operations[], trx: Transaction) => {
  const ids = []
  const timestamps = []
  const accounts = []
  const proxys = []
  ...
  await trx.queryObject`INSERT INTO hafsql.operation_proxy_cleared_table
    (id, timestamp, account, proxy)
    SELECT UNNEST(${ids}::int8[]), UNNEST(${timestamps}::timestamp[]),
    UNNEST(${accounts}::text[]), UNNEST(${proxys}::text[])
    ON CONFLICT (id) DO NOTHING;`
