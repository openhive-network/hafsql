...
export const setupOperationIndexes = async () => {
  ...
  // required_auths
  await pool.query(
    `CREATE INDEX ${CONCURRENTLY} IF NOT EXISTS hafsql_required_auths_idx ON hive.operations ((body::jsonb->'value'->'required_auths'), op_type_id, id DESC)
      WHERE op_type_id IN (15, 18);`
  )
  ...
  await pool.query(
    `CREATE INDEX ${CONCURRENTLY} IF NOT EXISTS hafsql_required_posting_auths_idx ON hive.operations ((body::jsonb->'value'->'required_posting_auths'), op_type_id, id DESC)
      WHERE op_type_id IN (15, 18);`
  )
