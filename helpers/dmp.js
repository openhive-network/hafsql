...
diff_match_patch.prototype.diff_halfMatch_ = function (text1, text2) {
  ...
  const hm1 = diff_halfMatchI_(longtext, shorttext,
    Math.ceil(longtext.length / 4))
  // Check again based on the third quarter.
  const hm2 = diff_halfMatchI_(longtext, shorttext,
...
diff_match_patch.prototype.diff_cleanupSemanticLossless = function (diffs) {
  ...
  while (pointer < diffs.length - 1) {
    if (diffs[pointer - 1][0] == DIFF_EQUAL &&
      ...
      const equality1 = diffs[pointer - 1][1]
      const edit = diffs[pointer][1]
      const equality2 = diffs[pointer + 1][1]
      const buffer = equality1 + edit + equality2

      // First, shift the edit as far left as possible.
      const offsetLeft = this.diff_commonSuffix(equality1, edit)
      const offsetRight = this.diff_commonPrefix(edit, equality2)
      const originalEditStart = equality1.length
      let editStart = originalEditStart - offsetLeft
      const maxEditStart = originalEditStart + offsetRight
      let editEnd = editStart + edit.length

      // Second, step character by character right, looking for the best fit.
      let bestEditStart = editStart
      let bestEditEnd = editEnd
      let bestScore = diff_cleanupSemanticScore_(buffer, editStart) +
          diff_cleanupSemanticScore_(buffer, editEnd)
      while (editStart < maxEditStart) {
        editStart += 1
        editEnd += 1
        const score = diff_cleanupSemanticScore_(buffer, editStart) +
          diff_cleanupSemanticScore_(buffer, editEnd)
