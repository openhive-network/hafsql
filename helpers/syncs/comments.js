...
const insertComments = async (items) => {
  ...
  for (let i = 0; i < commentsArray.length; i++) {
    ...
    queryString += `(${comment.author},${comment.permlink},${comment.op_id},${comment.bodyEdited},${body},'${comment.tags}'::jsonb)`
    if (first) {
      first = false
    }
  }
  console.log(queryString)
  await pool.query(`INSERT INTO hafsql.comments_table (author, permlink, last_op_id, body_edited, body, tags) VALUES ${queryString};`)
}
